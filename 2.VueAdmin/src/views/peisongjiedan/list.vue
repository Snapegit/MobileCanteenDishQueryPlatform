	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('peisongjiedan','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('peisongjiedan','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('peisongjiedan','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('peisongjiedan','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('peisongjiedan','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('peisongjiedan','完成')" type="success" @click="dingdanwanchengCrossAddOrUpdateHandler(scope.row,'cross','','peisongzhuangtai','已完成','已完成,配送中'.split(',')[0])">完成</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
    ...
    const dingdanwanchengCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			dingdanwanchengFormModelRef.value.init(row.id,'cross','完成',row,'peisongjiedan',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
